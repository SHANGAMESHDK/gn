...
export function CampusARHunt() {
  ...
  return (
    <div style={{ margin: 0, overflow: 'hidden', height: '100vh', width: '100vw', position: 'relative', backgroundColor: '#0f172a' }}>
      ...
      {/* Completion Modal */}
      {isComplete && (
        <div className="absolute inset-0 z-[60] bg-slate-900/80 backdrop-blur-sm flex items-center justify-center p-6 animate-in fade-in duration-500">
      ...
      >
        {/* Render uncollected Easter Eggs */}
        {EASTER_EGGS.filter(egg => !collectedEggs.includes(egg.id)).map((egg) => (
          <a-entity
            key={egg.id}
            class="clickable"
            look-at="[gps-camera]"
            gps-entity-place={`latitude: ${egg.lat}; longitude: ${egg.lng};`}
            scale="10 10 10"
