...
export function CampusAR() {
  ...
  // Maximum distance to render to prevent "dumping" and crowding
  ...
  return (
    <div style={{ margin: 0, overflow: 'hidden', height: '100vh', width: '100vw', position: 'relative', backgroundColor: '#000' }}>
      ...
      >
        {/* Render POIs using modern gps-new-entity-place */}
        {pois.map((poi) => {
          const isBuilding = poi.type === 'building';
          const color = isBuilding ? '#3b82f6' : '#10b981';
          
          // Scale down objects that are further away so they don't block everything
          const dynamicScale = Math.max(5, 20 - (poi.distance / 10));

          return (
            <a-entity
              key={poi.id}
              look-at="[gps-new-camera]"
              gps-new-entity-place={`latitude: ${poi.lat}; longitude: ${poi.lng};`}
        ...
        {/* Modern GPS Camera to fix dumping issues */}
        <a-camera
          gps-new-camera={simulationMode ? `simulateLatitude: ${SIMULATED_LAT}; simulateLongitude: ${SIMULATED_LNG}` : "gpsMinDistance: 5"}
          rotation-reader
        ></a-camera>
      </a-scene>
    </div>
